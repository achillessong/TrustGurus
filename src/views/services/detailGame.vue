    ...
    <div>
        <section class="relative">
            ...
            <div class="absolute left-1/2 top-[58px] w-full -translate-x-1/2 text-center text-white lg:top-[98px] xl:top-28">
                <h2 class="text-4xl font-black italic sm:text-6xl md:text-6xl xl:text-[150px]">{{jsonData[0]?.title}}</h2>
                <p
...
<script setup lang="ts">
    ...
    onMounted(async () => {
    try {
        const leadersResponse = await fetch('/json/detail-game.json');
        const stepProcessesResponse = await fetch('/json/stepProcessGame.json');
        const expertiseResponse = await fetch('/json/expertiseGame.json');
