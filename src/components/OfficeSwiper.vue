<template>
    <section class="bg-gradient-to-t from-white/[54%] to-transparent py-14 dark:bg-none lg:py-[100px]">
        <div class="container">
            ...
            <swiper-slide v-for="office in jsonData" :key="office?.id">
                <div class="mt-10 rounded-[32px] bg-white dark:bg-gray-dark md:grid md:grid-cols-2 lg:mt-0">
                    <img :src=office?.image alt="office" class="h-full w-full rounded-r-[32px] rounded-l-[32px] object-cover md:rounded-r-none" />
                    <div class="py-10 px-5 sm:px-8">
                        <div>
                            <img :src=office?.icon alt="office" class="h-5 w-10 rounded-r-[2px] rounded-l-[2px] object-cover md:rounded-r-none" />
                        </div>
                        <ul class="space-y-4 pt-10">
                            <li class="flex gap-3">
                                ...
                                <div class="text-sm font-bold sm:text-lg">{{office?.location}}</div>
                            </li>
                            <li class="flex items-center gap-3">
                                ...
                                <a href="tel:+15193726595" class="text-sm font-bold hover:text-secondary sm:text-lg">{{office?.phoneNumber}}</a>
                            </li>
                            <li class="flex items-center gap-3">
                                <div>
                                    <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                ...
                                <a href="mailto:info@TrustGurus.com" class="text-sm font-bold hover:text-secondary sm:text-lg">{{office?.email}}</a>
                            </li>
                        </ul>
                    </div>
                </div>
...
<script setup lang="ts">
    ...
    onMounted(async () => {
    try {
        const response = await fetch('/json/officer.json');
