            ...
            <div class="grid gap-y-10 gap-x-4 py-14 sm:grid-cols-3 lg:grid-cols-5 lg:py-[100px]">
                ...
                <div>
                    <ul class="flex flex-col gap-3 font-bold">
                        ...
                        <div v-for="service in services">
                            <li>
                                <router-link :to="`/servicesDetail/${service?.link}`" class="inline-block transition hover:scale-110 hover:text-secondary">{{service?.category}}</router-link>
                            </li>
                        </div>
                    </ul>
                </div>
                ...
                <div>
                    <ul class="flex flex-col gap-3 font-bold">
                        ...
                        <li>{{teaminfor?.address}}</li>
                        <li>
                            <a href="tel:+(617) 254-2333" class="inline-block transition hover:scale-110 hover:text-secondary">{{teaminfor?.phonenumber}}</a>
                        </li>
                    </ul>
                </div>
            </div>
...
<script setup lang="ts">
    ...
    onMounted(async () => {
    try {
        const servicesResponse = await fetch('/json/categories.json');
        const teaminofrResponse = await fetch('/json/team_infor.json');
