            ...
            <div class="relative">
                <div class="container">
                    <div class="relative items-center py-14 lg:flex lg:py-[100px]">
                        <div class="relative z-[1] py-5 px-10 text-center text-white lg:w-2/5 ltr:lg:text-left rtl:lg:text-right xl:w-3/5">
                            <h2 class="text-4xl font-black sm:text-5xl sm:leading-[126px] xl:text-[100px]">{{jsonData[0]?.title}}</h2>
                            <p class="my-8 text-xl leading-[40px] text-white">
                                {{jsonData[0]?.description}}
                            </p>
                            <router-link to="/about-us" class="btn mt-2 capitalize text-white">{{jsonData[0]?.buttonDescripton}}</router-link>
                        </div>
                        <div
                            class="top-[100px] mt-10 h-200 w-auto ltr:right-0 rtl:left-0 lg:absolute lg:mt-0 lg:h-auto lg:w-2/5"
                            :data-aos="store.direction === 'rtl' ? 'fade-right' : 'fade-left'"
                            data-aos-duration="1000"
                        >
                            <img :src="jsonData[0]?.image" alt="services-detail-phone" class="mx-auto h-full" />
                        </div>
                    </div>
                </div>
            </div>
...
<script setup lang="ts">
    ...
    onMounted(async () => {
    try {
        const leadersResponse = await fetch('./public/json/detail-degital.json');
        const stepProcessesResponse = await fetch('./public/json/stepProcessDigital.json');
        const expertiseResponse = await fetch('./public/json/expertiseDegital.json');
